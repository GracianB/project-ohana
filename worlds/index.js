...
export function renderWorld(ctx, world, cam, t, W, H) {
  ...
  if (world.id === "beach") {
    ctx.fillStyle = "#4aa3e8"; ctx.fillRect(0, H * 0.62, W, H);
    ctx.fillStyle = "rgba(255,255,255,.7)";
    for (let i = 0; i < 5; i++) { ctx.beginPath(); ctx.ellipse((i * 280 + t * 0.2) % W, 70 + i * 16, 40, 14, 0, 0, Math.PI * 2); ctx.fill(); }
  }
  if (world.id === "jungle") {
    for (let i = 0; i < 10; i++) {
      const x = ((i * 160 - cam.x * 0.4) % (W + 160));
      ctx.fillStyle = "#163816"; ctx.fillRect(x, H * 0.2, 28, H);
      ctx.fillStyle = "#2f7a2c"; ctx.beginPath(); ctx.arc(x + 14, H * 0.22, 36, 0, Math.PI * 2); ctx.fill();
    }
  }
  if (world.id === "volcano") {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      ctx.beginPath(); ctx.moveTo(x, H * 0.7); ctx.lineTo(x + 90, H * 0.3); ctx.lineTo(x + 180, H * 0.7); ctx.fill();
    }
    ctx.fillStyle = "#ff5a12"; ctx.globalAlpha = 0.5; ctx.fillRect(0, H * 0.72, W, 16); ctx.globalAlpha = 1;
  }
  if (world.id === "space") {
    ctx.fillStyle = "#fff";
    for (let i = 0; i < 70; i++) ctx.fillRect((i * 97 + cam.x * 0.05) % W, (i * 53) % H, 2, 2);
    ctx.fillStyle = "#6a3cff"; ctx.beginPath(); ctx.arc(W * 0.75, 120, 40, 0, Math.PI * 2); ctx.fill();
  }
  if (world.id === "lab") {
