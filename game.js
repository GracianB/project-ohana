...
function makePlayer(def) {
  return { ...def, x: game.spawn.x, y: game.spawn.y, vx: 0, vy: 0, facing: 1, jumps: 0, grounded: false, health: def.health, maxHealth: def.health, evo: 0, dead: false, invuln: 0, cds: {}, gliding: 0, xp: 0 };
}
...
function start(def) {
  ...
  renderAbilityBar();
  showNotification("OHANA", def.name + " listo. J/K/L · E evoluciona · 1-5 mundos · ojo a los huecos");
}

function evolve() {
  const p = game.player;
  if (!p || p.dead) return;
  if (p.evo >= 2) { showNotification("MAX", "Ya estas en la forma final."); return; }
  p.evo++;
  p.maxHealth = Math.round(p.maxHealth * 1.28);
  p.health = p.maxHealth;
  p.speed += 0.55;
  showNotification("EVOLUCION", p.evoNames[p.evo]);
  game.fx.emit(p.x + p.w / 2, p.y, { color: p.color, count: 32, size: 5, up: 1.4 });
}
...
function drawEnemy(e) {
  const x = e.x - game.cam.x;
  const y = e.y - game.cam.y;
  ctx.fillStyle = e.color;
  if (e.kind === "bat") {
    ctx.beginPath(); ctx.ellipse(x + 15, y + 16, 14, 8, 0, 0, Math.PI * 2); ctx.fill();
  } else if (e.kind === "bot") {
    ctx.fillRect(x + 4, y + 4, 22, 22);
  } else {
    ctx.beginPath(); ctx.ellipse(x + 15, y + 20, 15, 12, 0, 0, Math.PI * 2); ctx.fill();
  }
...
function setupSelect() {
  ...
  wrap.innerHTML = ROSTER.map((c) => '<div class="char-card" data-id="' + c.id + '"><div style="height:48px;background:' + c.color + ';border-radius:8px"></div><h3>' + c.name + "</h3><small>SPD " + c.speed + " · HP " + c.health + " · saltos " + c.maxJumps + "</small></div>").join("");
